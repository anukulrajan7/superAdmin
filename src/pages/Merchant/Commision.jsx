...
const Commision = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch("http://localhost:7000/api/v1/merchantCommission");
      if (response.ok) {
                ...
                <tbody>
                  {currentData.map((d, index) => (
                    ...
                    >
                      ...
                      <td className="merchant-table">
                        {new Date(d.dated_from).toLocaleDateString()}
                      </td>
                      <td className="merchant-table">
                        {new Date(d.dated_to).toLocaleDateString()}
                      </td>
                      <td className="merchant-table">
                        <div className="flex">
                          <HiPencil
